...
describe("Webhook", () => {
    ...
    const mockWalletActivityWebhookModel = {
        id: "test-id",
        network_id: "test-network",
        notification_uri: "https://example.com/callback",
    ...
    const mockContractActivityWebhookModel = {
        id: "test-id",
        network_id: "test-network",
        notification_uri: "https://example.com/callback",
    ...
    describe(".init", () => {
        it("should throw an error if the model is null", () => {
            expect(() => webhook_1.Webhook.init(null)).toThrow("Webhook model cannot be empty");
        });
        it("should create an instance of Webhook", () => {
            const webhook = webhook_1.Webhook.init(mockModel);
            expect(webhook).toBeInstanceOf(webhook_1.Webhook);
        });
    });
    describe("#getId", () => {
        ...
        it("should return undefined if the ID is not set", () => {
            ...
            const webhook = webhook_1.Webhook.init(modelWithoutId);
    ...
    describe("#getNetworkId", () => {
        ...
        it("should return undefined if the network ID is not set", () => {
            const modelWithoutNetworkId = {
                ...mockModel,
                network_id: undefined,
            };
            const webhook = webhook_1.Webhook.init(modelWithoutNetworkId);
    ...
    describe("#getNotificationURI", () => {
        ...
        it("should return undefined if the notification URI is not set", () => {
            const modelWithoutNotificationURI = {
                ...mockModel,
                notification_uri: undefined,
            };
            const webhook = webhook_1.Webhook.init(modelWithoutNotificationURI);
    ...
    describe("#getEventType", () => {
        ...
        it("should return undefined if the event type is not set", () => {
            const modelWithoutEventType = {
                ...mockModel,
                event_type: undefined,
            };
            const webhook = webhook_1.Webhook.init(modelWithoutEventType);
    ...
    describe("#getEventFilters", () => {
        ...
        it("should return undefined when event filters are not set", () => {
            const modelWithoutFilters = {
                ...mockModel,
                event_filters: undefined,
            };
            const webhook = webhook_1.Webhook.init(modelWithoutFilters);
    ...
    describe("#getSignatureHeader", () => {
        it("should return undefined since the signature header can not be set via SDK", () => {
            const modelWithoutSignatureHeader = {
                ...mockModel,
                signature_header: undefined,
            };
            const webhook = webhook_1.Webhook.init(modelWithoutSignatureHeader);
            expect(webhook.getSignatureHeader()).toBeUndefined();
        });
    });
    describe(".create", () => {
        it("should create a new webhook", async () => {
            const webhook = await webhook_1.Webhook.create({
                networkId: "test-network",
                notificationUri: "https://example.com/callback",
                eventType: "erc20_transfer",
                eventFilters: [{ contract_address: "0x...", from_address: "0x...", to_address: "0x..." }],
            });
            expect(coinbase_1.Coinbase.apiClients.webhook.createWebhook).toHaveBeenCalledWith({
                network_id: "test-network",
                notification_uri: "https://example.com/callback",
                event_type: "erc20_transfer",
                event_filters: [{ contract_address: "0x...", from_address: "0x...", to_address: "0x..." }],
        ...
        it("should throw an error if creation fails", async () => {
            coinbase_1.Coinbase.apiClients.webhook.createWebhook = jest
                .fn()
                .mockRejectedValue(new Error("Failed to create webhook"));
            await expect(webhook_1.Webhook.create({
                networkId: "test-network",
                notificationUri: "https://example.com/callback",
                eventType: "erc20_transfer",
    ...
    describe(".list", () => {
        it("should list all webhooks", async () => {
            const paginationResponse = await webhook_1.Webhook.list({ limit: 1 });
            const webhooks = paginationResponse.data;
            expect(coinbase_1.Coinbase.apiClients.webhook.listWebhooks).toHaveBeenCalledWith(1, undefined);
        ...
        it("should throw an error if list fails", async () => {
            coinbase_1.Coinbase.apiClients.webhook.listWebhooks = (0, utils_1.mockReturnRejectedValue)(new api_error_1.APIError(""));
            await expect(webhook_1.Webhook.list()).rejects.toThrow(api_error_1.APIError);
        });
    });
    describe("#update", () => {
        ...
        it("should update the webhook address list only", async () => {
            ...
            expect(webhook.getNotificationURI()).toBe("https://example.com/callback");
            expect(webhook.getEventTypeFilter()?.addresses).toEqual([
                "0x1..",
                "0x2..",
            ]);
        });
        it("should update both the webhook notification URI and the list of addresses monitoring", async () => {
            const webhook = webhook_1.Webhook.init(mockWalletActivityWebhookModel);
        ...
        it("should update notification URI for contract webhook", async () => {
            const webhook = webhook_1.Webhook.init(mockContractActivityWebhookModel);
            await webhook.update({
            ...
            expect(webhook.getNotificationURI()).toBe("https://new-url-for-contract-webhook.com/callback");
            expect(webhook.getEventTypeFilter()).toEqual({
                contract_addresses: ["0xa55C5950F7A3C42Fa5799B2Cac0e455774a07382"],
            });
        });
    ...
    describe("#delete", () => {
        it("should delete the webhook and set the model to null", async () => {
            ...
            expect(coinbase_1.Coinbase.apiClients.webhook.deleteWebhook).toHaveBeenCalledWith("test-id");
            expect(webhook.getId()).toBeUndefined();
            expect(webhook.getNetworkId()).toBeUndefined();
        });
    });
    describe("#toString", () => {
        it("should return a string representation of the webhook", () => {
            ...
            expect(stringRepresentation).toBe(`Webhook { id: 'test-id', networkId: 'test-network', eventType: 'erc20_transfer', eventFilter: [{"contract_address":"0x...","from_address":"0x...","to_address":"0x..."}], eventTypeFilter: undefined, notificationUri: 'https://example.com/callback', signatureHeader: 'undefined' }`);
        });
    });
});
